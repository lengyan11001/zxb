...
规则：
1. 第一通电话只做开场、建立可信度、确认需求、约见面；
2. 不使用“打扰、方便、占用、免费评估、没过不收费、全程代办”等话术；
3. 每句话尽量短，口语化，避免IPO、资本化、闭环等虚词；
4. 必须引用企业画像中的具体线索；
5. 结尾必须使用二选一邀约；
6. 输出JSON：{"full":"完整话术","concise":"精简话术","opening":"前10秒开场","hookPoints":[],"keyClues":[],"objectionPrep":[],"structureRatio":{}}`;
...
async function generateScript(product, enterprise, variation = 0) {
  if (env.deepseekApiKey) {
    try {
      return await generateWithDeepSeek(product, enterprise, variation);
    } catch (err) {
...
async function generateWithDeepSeek(product, enterprise, variation) {
...
产品利益：${asArray(product.benefits).join('、')}
...
联系人：${enterprise.contact_person || enterprise.contactPerson || '负责人'}
信号：${signals.map((s) => `${s.label || s.type}：${s.evidence || s.description}`).join('；')}

请生成第一通电话话术。`;

  const res = await axios.post(env.deepseekApiUrl, {
  ...
  return normalizeScript(content, 'deepseek');
}

function generateLocal(product, enterprise) {
  const signals = asArray(enterprise.signals);
  const primary = signals[0];
  const contact = enterprise.contact_person || enterprise.contactPerson || '负责人';
  const productName = product.name || '科技金融方案';
  const clue = primary?.evidence || `${enterprise.industry || '科技'}企业，具备进一步核验价值。`;
  ...
  const opening = `${contact}您好，我是科技企业服务顾问。看到${enterprise.name}在${enterprise.industry || '科技服务'}方向有布局，想跟您确认一个政策和融资机会。`;
  const full = `${opening}
...
我建议先约20分钟，把贵司目前材料和可走路径过一遍。您看明天上午，还是后天下午更合适？`;

  return normalizeScript({
    full,
    concise: `${contact}您好，看到${enterprise.name}有${enterprise.industry || '科技'}相关线索：${clue}。我们做${productName}，先帮企业判断认定和融资路径。明天上午或后天下午，哪个时间方便简单过一下？`,
    opening,
    hookPoints: [primary?.label || '企业画像切入', productName],
    keyClues: signals.slice(0, 3).map((s) => s.evidence || s.description || s.label),
    objectionPrep: [
      '已经有合作方：可以作为第二方案做交叉验证，不影响现有合作。',
      '暂时不需要：先判断条件和额度空间，企业有数后再决定。',
      '怕麻烦：前期只需要基础材料清单，先做20分钟诊断。',
    ],
    structureRatio: { 开场: 15, 线索: 25, 价值: 35, 邀约: 25 },
  }, 'local');
}
...
function normalizeScript(content, provider) {
  return {
    ...
    structureRatio: content.structureRatio || { 开场: 15, 线索: 25, 价值: 35, 邀约: 25 },
    provider,
  };
}
